...
| Trigger Rule 资产 | ✅ | REST 管理自动化路由资产；Webhook/Cron 真正执行留到 v0.2 |
...
| 端到端闭环 | ✅ | 创建 → Skill 挂载 → Docker 沙箱 → 只读自动放行 → `kubectl apply` 挂起 → 精确审批 → 审计 → 自动销毁 |

### 明确不在 v0.1（留待 v0.2+）

Webhook/Cron 自动执行、Vault JIT 真实对接、真实 OpenCode 引擎接入、完整 Skill IDE、
...
│   │   ├── opencode/                  # v0.2 真实 OpenCode client 的接口契约
...
会话列表页还提供轻量资产面板：可写入示例 Skill / Trigger Rule，并在拉起手工会话时选择
Skill。选中的 `SKILL.md` 会由控制面物化到 `runtime_provider.config.workspace_root`，
...
端到端脚本会验证：匿名访问被拒绝、内部接口鉴权、CORS、Skill/Trigger Rule 资产 API、
带 Skill 创建会话、沙箱内 SKILL.md 只读挂载、Docker 沙箱创建、两条只读命令自动 APPROVE、
写命令 SUSPEND、人工精确放行、DB 与 API 审计计数以及沙箱自动销毁。
