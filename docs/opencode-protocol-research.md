...
> 当前代码已提供 `server/internal/opencode/client.HTTPEngine`，真实 CLI 镜像安装和运行切换
> 仍由部署方按环境完成。
...
接入真实 OpenCode 后，控制面可启用 `server/internal/opencode/client.HTTPEngine`；
Harness 控制面通信仍可再按 proto 契约升级为 gRPC 双向流。
...
- [x] 实现 Go HTTP/SSE client adapter：`server/internal/opencode/client.HTTPEngine`
