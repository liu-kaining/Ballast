...
server:
  ...
  jwt_secret: "${BALLAST_JWT_SECRET}"
  admin_token: "${BALLAST_ADMIN_TOKEN}"
  internal_token: "${BALLAST_INTERNAL_TOKEN}"
  cors_allowed_origins:
    - "https://ballast.example.com"
  cookie_secure: true
...
runtime_provider:
  ...
  config:
    ...
    control_plane_url: "https://ballast-control.example.com"
...
# command 是可执行文件，args 是参数数组，unsafe 表示检测到 shell 复合语法。
decision = "DENY" {
    input.unsafe
}
...
decision = "APPROVE" {
    ...
    {"get", "logs", "describe", "top"}[input.args[0]]
}

decision = "APPROVE" {
    not input.unsafe
    input.command == "terraform"
    {"plan", "validate"}[input.args[0]]
}

decision = "DENY" {
    {"mkfs", "fdisk", "dd", "shutdown", "reboot"}[input.command]
}
...
decision = "DENY" {
    input.command == "kubectl"
    input.args[0] == "delete"
    {"namespace", "clusterrolebinding"}[input.args[_]]
}
